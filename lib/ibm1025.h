...
static const unsigned short ibm1025_2uni[256] =
{
...
static int
ibm1025_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
   unsigned char c = *s;
...
static const unsigned char ibm1025_page00[] = {
...
static const struct gap ibm1025_idx[] =
{
...
static int
ibm1025_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, int n)
{
   struct gap* p2gap = ibm1025_idx;
   for (; p2gap->start < 0xffff ; p2gap++) {
      if (wc >= p2gap->start && wc <= p2gap->end) {
         *r = ibm1025_page00[wc - p2gap->idx];
         return 1;
      }
   }
   return RET_ILUNI;
