...
static const unsigned short ibm1130_2uni[256] =
{
...
static int
ibm1130_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, size_t n)
{
   unsigned char c = *s;
   *pwc = (ucs4_t) ibm1130_2uni[c];
   return 1;
}

static const unsigned char ibm1130_page00[] = {
...
static const struct gap ibm1130_idx[] =
{
...
static int
ibm1130_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, size_t n)
{
   const struct gap* p2gap = ibm1130_idx;
   for (; p2gap->start < 0xffff ; p2gap++) {
      if (wc >= p2gap->start && wc <= p2gap->end) {
         *r = ibm1130_page00[wc - p2gap->idx];
         return 1;
      }
   }
   return RET_ILUNI;
