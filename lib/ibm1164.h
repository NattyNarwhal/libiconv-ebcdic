...
static const unsigned short ibm1164_2uni[256] =
{
...
static int
ibm1164_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, size_t n)
{
   unsigned char c = *s;
   *pwc = (ucs4_t) ibm1164_2uni[c];
   return 1;
}

static const unsigned char ibm1164_page00[] = {
...
static const struct gap ibm1164_idx[] =
{
...
static int
ibm1164_wctomb (conv_t conv, unsigned char *r, ucs4_t wc, size_t n)
{
   const struct gap* p2gap = ibm1164_idx;
   for (; p2gap->start < 0xffff ; p2gap++) {
      if (wc >= p2gap->start && wc <= p2gap->end) {
         *r = ibm1164_page00[wc - p2gap->idx];
         return 1;
      }
   }
   return RET_ILUNI;
